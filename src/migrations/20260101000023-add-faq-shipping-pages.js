...
// Inserts the FAQ and Shipping & Delivery pages for existing installs (fresh
// installs get them from src/seeders/20260101000016-seed-faq-shipping-pages.js
// instead, since seeders don't re-run against an already-seeded database).
...
const PAGES = [
  { slug: 'faq', content: { seoTitle: 'FAQ — Lugo Tailoring', seoDescription: 'Frequently asked questions about ordering, bespoke vs. made-to-measure, payment, and more.', eyebrow: 'Help', heading: 'Frequently Asked Questions', lastUpdated: LAST_UPDATED, body: FAQ_BODY } },
  { slug: 'shipping-delivery', content: { seoTitle: 'Shipping & Delivery — Lugo Tailoring', seoDescription: 'How pickup and delivery works for Lugo Tailoring garments.', eyebrow: 'Help', heading: 'Shipping & Delivery', lastUpdated: LAST_UPDATED, body: SHIPPING_BODY } },
];
  ...
  up: async (queryInterface) => {
    ...
    for (const page of PAGES) {
      ...
      const [existing] = await queryInterface.sequelize.query('SELECT id FROM pages WHERE slug = ?', {
        replacements: [page.slug],
      });
      if (existing.length) continue;

      // eslint-disable-next-line no-await-in-loop
      await queryInterface.bulkInsert('pages', [
        { slug: page.slug, content: JSON.stringify(page.content), created_at: now, updated_at: now },
      ]);
    }
  },
